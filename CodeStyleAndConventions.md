...
## Mini-Game API Pattern

All mini-games must follow a standardized API pattern for integration with the lobby:

### Required Functions

```c
// Opaque game type
typedef struct GameName_St GameName_St;

// Initialize game with optional configuration
GameName_St* gameName_initGame(const GameConfig_St* config);

// Run one frame of game logic + rendering
void gameName_gameLoop(GameName_St* const game);

// Free all resources
void gameName_freeGame(GameName_St* game);

// Check if game is still running
bool gameName_isRunning(const GameName_St* game);
```

### Configuration

Games accept an optional `GameConfig_St*` parameter:
- If `NULL`, use default settings
- Check `config->video` and `config->audio` pointers before accessing (they may be NULL)
- Use provided macros: `VIDEO_CONFIG_DEFAULT`, `AUDIO_CONFIG_DEFAULT`

### Error Handling

- Return `NULL` from `initGame` on failure (lobby will handle gracefully)
- Use `GameError_Et` codes for detailed error reporting when needed
- Log errors using the project's logger before returning NULL

### Window Management

- Check `IsWindowReady()` before calling `InitWindow()`
- If window already exists (lobby case), use `SetWindowSize()` and `SetWindowTitle()` instead
- Store original window dimensions to restore on exit
- Track `windowInitialized` flag to know whether to call `CloseWindow()`

### Adding a New Game to Lobby

1. Create API header in `firstparty/APIs/gameNameAPI.h`
2. Implement the four required functions
3. Add entry to `MiniGame_St` array in `lobby/src/main.c`:
```c
{
    .name = "GAME_NAME",
...
```
4. Add corresponding enum value to `GameScene_Et`