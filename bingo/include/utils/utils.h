 ...
 */
#define shuffleArrayXor(array, size) \
do { \
    if ((size) > 1) { \
        for (u32 i = (u32)(size) - 1; i > 0; --i) { \
            u32 r = prng_rand() % (i + 1); \
 ...
 */
#define da_shuffleXor(da) shuffleArrayXor((da)->items, (da)->count)

/**
 * @brief Shuffles a dynamic array's items using Type swap.
 */
#define da_shuffle(T, da, randFunc) shuffleArray(T, (da)->items, (da)->count, randFunc)
