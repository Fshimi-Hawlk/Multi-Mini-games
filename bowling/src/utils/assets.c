...
Error_Et bowling_loadTextures(BowlingTextures_St* textures) {
    ...
    Image ballImage = LoadImage(BALL_TEXTURE);
    if (!IsImageValid(ballImage)) {
        log_error("Failed to load ball image: %s", BALL_TEXTURE);
    ...
    if (extractedCount <= 0) {
        log_warn("Ball extraction failed, loading whole image as single texture");
        textures->ballTextures[0] = LoadTextureFromImage(ballImage);
        if (!IsTextureValid(textures->ballTextures[0])) {
            log_error("Failed to load fallback ball texture");
            UnloadImage(ballImage);
            return ERROR_ASSET_LOAD;
        }
        // Build fallback model too
        Mesh sphere = GenMeshSphere(1.0f, 24, 24);
        textures->ballModels[0] = LoadModelFromMesh(sphere);
        SetMaterialTexture(&textures->ballModels[0].materials[0],
                           MATERIAL_MAP_DIFFUSE, textures->ballTextures[0]);
        textures->ballTextureCount = 1;
        textures->ballModelCount   = 1;
    } else {
    ...
    textures->pinTexture = LoadTexture(PIN_TEXTURE);
    if (!IsTextureValid(textures->pinTexture))
        log_warn("Failed to load pin texture: %s", PIN_TEXTURE);

    // ── Lane texture (non-fatal) ───────────────────
    textures->laneTexture = LoadTexture(LANE_TEXTURE);
    if (!IsTextureValid(textures->laneTexture))
        log_warn("Failed to load lane texture: %s", LANE_TEXTURE);

    // ── Crowd OBJ model ────────────────────────────
    textures->crowdModel = LoadModel(CROWD_MODEL);
    if (textures->crowdModel.meshCount > 0) {
        ...
        log_warn("Could not load crowd model: %s", CROWD_MODEL);
    }
