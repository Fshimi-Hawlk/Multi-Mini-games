...
1. **Include General API**:
    - Use `#include "APIs/generalAPI.h"` for shared types like `Error_Et` and `BaseGame_St`.
...
2. **Opaque Game Structure**:
    ...
    - Ensure it embeds `BaseGame_St` fields (e.g., `running`) at the start for casting compatibility (see notes in `gameNameAPI.h`).
...
5. **Function Prototypes**:
    ...
    - `Error_Et <gamename>_gameLoop(<GameName>Game_St* const game);` - Handles input, update, render.
    - `Error_Et <gamename>_freeGame(<GameName>Game_St** game);` - Frees resources.
...
6. **Guards and Docs**:
    ...
    - Add Doxygen comments for functions and file.
        Note: Refer to `CodeStyleAndConventions.md` for more info.
 ...
 *       `((BaseGame_St*)game)->running = false;`
 */
...
/**
 * @brief Concrete Tetris game state - embeds BaseGame_St as first member.
 */
...
**Note**:
- Creation of `miniRef` avoid to constantly cast to `BaseGame_St*` for general fields accessing like `running`.
