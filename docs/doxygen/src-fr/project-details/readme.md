...
2. Chaque nouveau mini-jeux se développe sur sa propre branche dédiée** (exemples: "block-blast", "snake-classic", "tetris", "memory-game", etc.).
...
4. Une fois fusionné, le dossier de jeu devient permanent sur `main` et nous travaillons à le brancher au lobby.

- Nouveau modèle de démarrage de jeu: regardez le dossier `sous-projet-exemple/`
Il contient déjà la mise en page recommandée du dossier, Makefile, Doxyfile minimal, `makefile.md` et un talon README.
...
Après avoir fusionné plus de jeux, vous verrez des dossiers de haut niveau supplémentaires comme `block-blast/`, `snake-classic/`, etc.
...
Liste complète des commandes et modes (debug, sanitizers, valgrind, etc.) expliqués dans @ref root makefile.

## Construction et fonctionnement (dans un seul sous-projet / branche)

De l'intérieur `sous-projet-exemple/`, `lobby/`, `tetris/`, ou tout dossier de jeu:
...
Remarque : le Makefile de niveau racine pour construire *tout* à la fois est toujours WIP — suivre les progrès dans @ref todo.
...
-> ouvrir "docs/doxygen/index.html"
