...
- `main` -> squelette précoce + quelques docs, mais complètement dépassé maintenant. On a cessé de le toucher il y a des mois.
- Directions du développement actif :
- Les branches de jeu individuelles (par exemple "tetris", "snake-classic", "block-blast", "memory", etc.) -> où les jeux monojoueurs sont construits et polis autonomes
- `docs-setup` -> documentation partagée, règles de style de code, guides de configuration, petits refacteurs qui touchent plusieurs parties
- `sous-jeux-intégration-test` (on l'appelle `sgit`) -> la branche d'intégration principale actuelle. Nous fusionnons des jeux en un seul joueur pour les brancher dans le hall, les tester ensemble, corriger les bugs du hall, ajouter des fonctionnalités partagées

En ce moment : plusieurs jeux fonctionnent de manière autonome, un couple est déjà jouable dans le lobby via "sgit". Le lobby lui-même a un mouvement basique, des déclencheurs de collision et de l'interface utilisateur, mais il est encore rugueux autour des bords.
...
- Intégrez-les un à un dans le hall par le biais de "git"
- Une fois que nous avons 4-5+ jeux bien travailler ensemble -> peut-être renommer `sgit` quelque chose de plus clair comme `singleplayer-games-integration-test`

2. **La phase multijoueur** (prochaine étape majeure après un seul joueur se sent solide)
- Créer une nouvelle branche : `multiplayer-games-integration-test` (court : `mgit`)
...
- Créer `combine-games-intégration-test` (court: `cgit`)
...
- Quand `cgit` est jouable et raisonnablement stable -> fusionner tout avec `main`
...
- Si le multijoueur prend plus de temps que prévu, nous pouvons envoyer une seule version de lecteur à `main` d'abord
