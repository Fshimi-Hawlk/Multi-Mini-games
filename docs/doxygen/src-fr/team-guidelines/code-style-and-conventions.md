...
- ` St` pour les structures régulières (y compris les tableaux cultivables) -> `PlayerArray St`
- ` Et` pour les enums -> `GameScène Et`
- "Ut" pour les syndicats
- ` Ft` pour les types de fonctions
- ` t` comme repli si rien d'autre ne correspond

Nous avons des pseudonymes dans `baseTypes.h` pour le code normal (lorsque la taille exacte n'est pas critique):
...
Ligne directrice:
Utiliser des types de largeurs fixes (`u32`, `s32`, etc.) lorsque la taille/signalité est importante (réseautage, sérialisation, bit ops).
Utilisez les petits (`uint`, `iVector2`, etc.) partout ailleurs pour un gameplay plus propre/code UI.
Évitez les "int"/`unsigned int`/`float` dans un nouveau code à moins qu'une lib de la 3ème partie ne le force.
...
Utilisez `static` pour tout ce qui devrait rester privé d'un fichier .c:

- fonctions "statiques" si elles ne font pas partie de l'API publique
- variables/globales du champ de fichiers `static`
...
- Espace après mots-clés: `si (`, `alors (`, `pour (`
- Espace autour des opérateurs binaires: `a + b`, `x = y`
- Conseils de pointeur : `Type* ptr` (pointeur du type), `Type *array` (pointeur du type)
- placement const: préfèrent `const Type* ptr` quand pointee ne devrait pas changer
- Pas de virgules dans les listes de param, "vitez func()" pour les params vides
...
Constantes: CASE ÉCRANGE -> `#define MAX PLAYERS 4'
...
En-tête : style `#ifndef PATH LIKE NAME H` -> `#ifndef LOBBY API H`
...
- Utiliser la macro `UNREACHABLE` pour les cas impossibles
- Contrôles défensifs null/bounds
- Connectez-vous à notre enregistreur (log info, log warn, etc.)
- `goto` ok pour le nettoyage dans les chemins d'erreur - ajouter un commentaire pourquoi
- Aucun nombre magique -> utiliser `#define` ou enums

## Gestion de la mémoire

Un faible code d'allocation peut utiliser un "malloc"/"calloc".
Préférez aréna allocators (`globalArena`, `tempArena`, `context alloc`) pour la plupart des choses - le suivi des fuites plus facile.
