/**
    @file test_scoring.c
    @author Fshimi-Hawlk
    @date 2026-04-10
    @brief Unit tests for scoring system.
*/

#include "core/game.h"
#include "core/board.h"
#include "core/placement.h"

#include "utils/globals.h"
#include "utils/userTypes.h"

#include <assert.h>

static void test_calculate_score(void) {
    Board_St testBoard = {.width = 8, .height = 8};
    testBoard.rowsToClear = context_alloc(testBoard.height * sizeof(bool));
    memset(testBoard.rowsToClear, 0, testBoard.height * sizeof(bool));
    testBoard.columnsToClear = context_alloc(testBoard.width * sizeof(bool));
    memset(testBoard.columnsToClear, 0, testBoard.width * sizeof(bool));

    ScoringState_St scoringState = {0};

    // No lines: 0
    manageScoreAndStreak(&scoringState, &testBoard, 0);
    assert(scoringState.score == 0.0f); // note: internal function, but used via manageScoreAndStreak in new code
    log_info("OK");

    // 1 line: 100
    testBoard.rowsToClear[0] = true;
    manageScoreAndStreak(&scoringState, &testBoard, 0);
    assert(scoringState.score == 100.0f);
    log_info("OK");

    updateBoardClearing(&testBoard);
    
    // 2 lines: 100 * 2 * 1.5 = 300
    testBoard.rowsToClear[1] = true;
    manageScoreAndStreak(&scoringState, &testBoard, 0);
    assert(scoringState.score == 300.0f);
    log_info("OK");

    updateBoardClearing(&testBoard);

    // Mixed row/col: 3 lines -> 100 * 3 * 2.0 = 600
    testBoard.columnsToClear[0] = true;
    manageScoreAndStreak(&scoringState, &testBoard, 0);
    assert(scoringState.score == 600.0f);
    log_info("OK");
}

static void test_manage_score(void) {
    GameState_St testGame = {0};
    testGame.board = (Board_St) {.width = 8, .height = 8};
    testGame.board.rowsToClear = context_alloc(testGame.board.height * sizeof(bool));
    testGame.board.columnsToClear = context_alloc(testGame.board.width * sizeof(bool));
    
    Shape_St mockShape = {.prefab = &prefabs[PREFAB_1x4]};

    // Base placement: +40 (no clear)
    placeShape(&mockShape, (u8Vector2) {0, 0}, &testGame.board);
    manageScoreAndStreak(&testGame.scoring, &testGame.board, mockShape.prefab->blockCount);
    clearBoard(&testGame.board);
    assert(testGame.scoring.score == 40);
    log_info("OK");

    // With clear: +40 + calculated
    placeShape(&mockShape, (u8Vector2) {0, 4}, &testGame.board);
    manageScoreAndStreak(&testGame.scoring, &testGame.board, mockShape.prefab->blockCount);
    clearBoard(&testGame.board);
    assert(testGame.scoring.score == 40 + 40 + 100);
    log_info("OK");
    assert(testGame.scoring.streakCount == 1);
    log_info("OK");
}

static void test_score_text(void) {
    GameState_St testGame = {0};
    testGame.scoring.score = 500;
    buildScoreRelatedTexts(&testGame.scoring);
    assert(strcmp(testGame.scoring.scoreText, "Score: 500") == 0);
    log_info("OK");
}

int main(void) {
    test_calculate_score();
    test_manage_score();
    test_score_text();

    arena_free(&tempArena);
    arena_free(&globalArena);

    log_info("Scoring tests passed");
    return 0;
}