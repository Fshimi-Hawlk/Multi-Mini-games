...
$frDir      = $doxygenDir . '/' . $lang; // translated .md sources
$doxyfileSrc = $doxygenDir . '/Doxyfile.fr'; // use FR Doxyfile as template
...
// ----------------------------------------------------------------
// Collect .md files to translate (skip mainpage.md — already FR)
// ----------------------------------------------------------------
function collectMd(string $dir, string $base): array {
    $files = [];
    $it = new RecursiveIteratorIterator(new RecursiveDirectoryIterator($dir));
    foreach ($it as $f) {
        if ($f->isFile() && $f->getExtension() === 'md' && $f->getFilename() !== 'mainpage.md') {
            $files[] = $f->getPathname();
        }
    }
    sort($files);
...
// Replace output dir html-fr -> html-{lang}
$doxyContent = str_replace('HTML_OUTPUT             = html-fr', 'HTML_OUTPUT             = html-' . $lang, $doxyContent);
// Replace OUTPUT_LANGUAGE = French -> auto (Doxygen uses English as fallback for unknown)
$doxyContent = str_replace('OUTPUT_LANGUAGE         = French', 'OUTPUT_LANGUAGE         = ' . ucfirst($lang), $doxyContent);
// Replace input fr -> {lang}
$doxyContent = str_replace("\nINPUT                   = fr", "\nINPUT                   = " . $lang, $doxyContent);
