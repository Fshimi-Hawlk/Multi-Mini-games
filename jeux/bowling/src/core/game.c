...
static bool bowling_handleTitleScreenInput(BowlingGame_St* game) {
    ...
    if (CheckCollisionPointRec(mp, ts->skinPrev)) {
        int maxSkin = game->textures.ballTextureCount;
        if (maxSkin > 0) ts->selectedSkin = (ts->selectedSkin - 1 + maxSkin) % maxSkin;
        return false;
    }
    if (CheckCollisionPointRec(mp, ts->skinNext)) {
        int maxSkin = game->textures.ballTextureCount;
        if (maxSkin > 0) ts->selectedSkin = (ts->selectedSkin + 1) % maxSkin;
        return false;
    }
...
Error_Et bowling_initGame__full(BowlingGame_St** game, BowlingConfigs_St configs) {
    ...
    *game = malloc(sizeof(BowlingGame_St));
    ...
    if (tex != OK) {
        log_error("Failed to load textures – aborting bowling initialization");
        free(g);
        *game = NULL;
        return tex;
    }
