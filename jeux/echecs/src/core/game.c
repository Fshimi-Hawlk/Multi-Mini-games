...
/**
 * @brief One frame: input, game logic updates, render (lobby / external driver).
 */
void echecsGameOneFrame(Board_t board) {
    if (!running) {
        return;
    }
 ...
 * @param nbPredifinedMoves Number of predefined moves
 */
void gameLoop(Board_t board, char *predifinedMoves[], int nbPredifinedMoves) {
    if (nbPredifinedMoves > 0 && predifinedMoves) {
        applyPredifinedMoves(board, predifinedMoves, nbPredifinedMoves);
    }

    while (running) {
        echecsGameOneFrame(board);
    }
}
