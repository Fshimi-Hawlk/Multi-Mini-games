 ...
 * 
 * Renders the cube with proper 3D perspective using raylib's 3D mode.
 * 
...
static void drawCube(const RubiksCubeGame_St* game) {
    ...
    float rotationAngle = angleY + angleX;
    
    Camera3D camera = {
        .position = {0, 2, game->zoom},
        .target = {0, 0, 0},
        .up = {0, 1, 0},
        .fovy = 45,
    ...
    float gap = 0.1f;
    float cellSize = faceSize / 3.0f;
    
    for (int faceIdx = 0; faceIdx < 6; faceIdx++) {
        float offsetX = 0, offsetY = 0, offsetZ = 0;
        
        switch (faceIdx) {
            case RUBIKS_FACE_FRONT:
                offsetZ = faceSize / 2 + gap;
                break;
            case RUBIKS_FACE_BACK:
                offsetZ = -faceSize / 2 - gap;
                break;
            case RUBIKS_FACE_RIGHT:
                offsetX = faceSize / 2 + gap;
                break;
            case RUBIKS_FACE_LEFT:
                offsetX = -faceSize / 2 - gap;
                break;
            case RUBIKS_FACE_TOP:
                offsetY = faceSize / 2 + gap;
                break;
            case RUBIKS_FACE_BOTTOM:
                offsetY = -faceSize / 2 - gap;
                break;
        }
        ...
        Vector3 facePos = {
            offsetX * cosA - offsetZ * sinA,
            offsetY,
            offsetX * sinA + offsetZ * cosA
        };
        
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
                Color color = getColor(game->faces[faceIdx][i][j]);
                
                Vector3 cellPos = {
                    facePos.x + (j - 1) * cellSize,
                    facePos.y + (1 - i) * cellSize,
                    facePos.z
                };
                
                float halfCell = cellSize / 2.0f - 1;
                
                DrawCubeV(cellPos, (Vector3){halfCell * 2, halfCell * 2, 2}, color);
                DrawCubeWiresV(cellPos, (Vector3){halfCell * 2, halfCell * 2, 2}, BLACK);
            }
        }
    }
...
static void handleInput(RubiksCubeGame_St* game) {
    ...
    // Zoom with Q/E or +/-
    if (IsKeyDown(KEY_Q) || IsKeyDown(KEY_EQUAL)) {
        game->zoom -= 0.1f;
        if (game->zoom < 3.0f) game->zoom = 3.0f;
    }
    if (IsKeyDown(KEY_E) || IsKeyDown(KEY_MINUS)) {
        game->zoom += 0.1f;
        if (game->zoom > 40.0f) game->zoom = 40.0f;
    }
