/**
    ...
    Functions are split between:
        ...
        - game zone logic (checkGameZones, isNearGameZone)
    ...
    @see `utils/types.h`      for Player_st, LobbyGame_St, Platform_st definitions
...
/**
    @brief Checks if player is colliding with any game zone and triggers scene switch.

    Iterates through all game zones and checks for collision with player.
    Sets subGameManager.currentScene and needGameInit when player enters a zone.
    Uses grace period to prevent instant re-triggering.
...
/**
    @brief Checks if player is within proximity threshold of any game zone.

    Used for visual feedback (highlighting zones when player approaches).

    @param player  Player state
    @param zones   Array of game zones
    @param count   Number of game zones
    @return true if player is near any game zone
 */
