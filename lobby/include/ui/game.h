/**
    ...
    the main gameplay elements of the lobby (player character, platforms, and game zones).
    ...
    @see `utils/types.h`      for LobbyGame_St, Player_st, Platform_st, GameZone_st definitions
*/
...
/**
    @brief Draws all game zones (mini-game triggers) in the lobby world.

    Renders each game zone as a semi-transparent rectangle with a label.
    Zones change appearance when player is nearby (highlight effect).

    @param zones    Array of game zone definitions
    @param count    Number of game zones in the array
 */
