...
typedef enum {
    ...
    GAME_SCENE_BOWLING,     ///< Bowling mini-game
    __gameSceneCount
} GameScene_Et;
...
/**
    @brief Game zone trigger that activates a mini-game when player enters.
 */
typedef struct {
    Rectangle hitbox;           ///< World-space collision rectangle
    GameScene_Et targetScene;   ///< Which mini-game this zone triggers
    Color     debugColor;       ///< Debug visualization color
    bool      isHighlighted;    ///< Visual feedback when player is near
} GameZone_st;
