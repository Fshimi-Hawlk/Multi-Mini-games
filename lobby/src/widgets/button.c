...
void textButtonDraw(const TextButton_St* btn, Font font, f32 fontSize) {
    ...
    switch (btn->state) {
        case WIDGET_STATE_HOVER: {
            bgColor = Fade(btn->baseColor, 0.85f);
        } break;

        case WIDGET_STATE_CLICK: {
            bgColor = Fade(btn->baseColor, 0.65f);
        } break;
        ...
        default: {
            bgColor = Fade(btn->baseColor, 0.95f);
        }
    }

    DrawRectangleRec(btn->bounds, bgColor);
    DrawRectangleLinesEx(btn->bounds, 2.0f, (btn->state == WIDGET_STATE_ACTIVE) ? YELLOW : DARKGRAY);
