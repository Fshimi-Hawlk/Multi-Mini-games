...
void textBoxDraw(const TextBox_St* box, Font font, f32 fontSize) {
    ...
    DrawRectangleRec(box->bounds, bgColor);
    DrawRectangleLinesEx(box->bounds, 2.0f,
        (box->state == WIDGET_STATE_ACTIVE) ? SKYBLUE : DARKGRAY);
    ...
    if (box->buffer[0] != '\0') {
        ...
        // Optional placeholder when empty and not focused
        DrawTextEx(font, "Enter text...", 
                   (Vector2){box->bounds.x + 8.0f, box->bounds.y + (box->bounds.height - fontSize) * 0.5f},
                   fontSize, 0.0f, GRAY);
    }
