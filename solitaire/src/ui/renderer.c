...
GameAssets LoadAssets(void) {
    ...
    assets.cardSheet = (Texture2D){0};
    for (int i = 0; assetPaths[i] != NULL; i++) {
        if (FileExists(assetPaths[i])) {
            assets.cardSheet = LoadTexture(assetPaths[i]);
            if (assets.cardSheet.id != 0) break;
        }
    }
    
    assets.cardBack = (Texture2D){0};
    for (int i = 0; backPaths[i] != NULL; i++) {
        if (FileExists(backPaths[i])) {
            assets.cardBack = LoadTexture(backPaths[i]);
            if (assets.cardBack.id != 0) break;
        }
    }
